...
def assign_grade(student: Student, subject: Subject, grade_string: str):
    ...
    grade_mapping = {'A': Grade.A, 'B': Grade.B, 'C': Grade.C, 'D': Grade.D, 'F': Grade.F}
    grade = grade_mapping.get(grade_string)
    
    if grade is not None and matched_enrollment.set_grade(grade):
        return 'Done'
    else:
        return 'Error'
...
def get_student_record(student: Student):
    ...
    for enrollment in matched_enrollments:
        subject_id = enrollment.get_subject().get_id()
        subject_name = enrollment.get_subject().get_name()
        ...
        record.update({subject_id: [subject_name, grade_name]})
...
def get_student_GPS(student: Student):
    ...
    matched_enrollments = search_subject_that_student_enrolled(student)
    for enrollment in matched_enrollments:
        total_grade += enrollment.get_grade().value * enrollment.get_subject().get_credit()
        total_weight += enrollment.get_subject().get_credit()
    gps = total_grade/total_weight
    return gps
