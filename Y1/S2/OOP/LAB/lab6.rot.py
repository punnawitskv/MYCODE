...
class User:
       def __init__(self, citizen_id, name, account_number, amount_of_money, atm_card):
              ...
              self.__atm_Card = ATM_Card(account_number, atm_card)
       @property
...
class ATM:
       ...
       def Deposit(self, atm, account, money):
              if money > 0 :
                     ...
                     self.transactions.append(f'D-ATM:{self.__number_atm}-{money}-{account.user.amount_of_money}')
                     return 'success'
              else : return 'error'
       ...
       def Withdraw(self, atm, account, money):
              if money > 0 and account.user.amount_of_money <= money :
                     account.user.amount_of_money -= money
                     self.transactions.append(f'W-ATM:{self.__number_atm}-{money}-{account.user.amount_of_money}')
                     return 'success'
              else : return 'error'   
       ...
       def Transfer(self, atm, account_own, account_transfer_to, money):
              if money > 0 and account_own.user.amount_of_money <= money :
                     ...
                     self.transactions.append(f'T-ATM:{self.__number_atm}-+{money}-{account_own.user.amount_of_money}')
                     return 'success'
              else : return 'error'   
...
user = {'1-1101-12345-49-0':['Harry Potter','1234567890',2000,'12345'],
...
print(f"Expected : {accout.user.atm_card}, {accout.user.account_number}, Success\n")
