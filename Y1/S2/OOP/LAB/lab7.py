class Bank:
    ...
    def search_seller(self, brand):
        for seller in self.__seller_list:
            if brand == seller.name:
                return seller
...
class Account:
    ...
    def __add__(self, amount):
        self.__amount += amount
        self.__transaction.append(f"D-ATM : {amount}-{self.__amount}")
        
    # def withdraw(self, amount):
    def __sub__(self , amount):
        self.__amount -= amount
        self.__transaction.append(f"W-ATM : {amount}-{self.__amount}")
    
    def transfer(self, unknow_no, amount, target_account):
        self.__amount -= amount
        target_account.amount += amount
        self.__transaction.append(f"T-ATM : {amount}-+{self.__amount}")
        target_account.transaction = f"T-ATM : {amount}-+{target_account.amount}"
        
    def paid(self, amount):
        self.__amount -= amount
        self.__transaction.append(f"P-aid : {amount}-+{self.__amount}")
        
    def receive(self, amount):
        self.__amount += amount
        self.__transaction.append(f"R-civ : {amount}-+{self.__amount}")
...
class ATM_machine:
    ...
    def withdraw(self, account, amount):
        if (self.__money >= amount) and (account.amount >= amount) and (amount > 0) and (amount <= ATM_machine.withdraw_limit):
            # account.withdraw(amount)
            account = account - amount
            self.__money -= amount
            
    def transfer(self,account, amount, target_account):
        # if (self.__money >= amount) and (amount <= 100000) and (account.amount >= amount) and (amount > 0):
...
class Seller:
    ...
    # paid(hermione_account,500,tops_account)
    def paid(self, account, amount, target_account):
        # account.transfer("0000", amount, target_account)
        account.paid(amount)
